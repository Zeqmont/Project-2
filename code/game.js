...
/// add "instant win" block, at the end of a complex jumping course to act as a win alternative to collecting the coins. (unique interactive)
/// add 
var actorChars = {
  "@": Player,
  "o": Coin, // A coin will wobble up and down
  "=": Lava, "|": Lava, "v": Lava, 
  "w": InstaWin // instant win block wobbles similar to the coin, different size
};
...
//add the instawin, coin alternative
function InstaWin(pos) {
	this.basePos = this.pos = pos. plus(new Vector (0.2, 0.1));
	this.size = new Vector (0.4, 0.4);
	this.wobble = Math.random() * Math.PI * 2;
}
InstaWin.prototype.type = "instawin";
...
InstaWin.prototype.act = function(step) {
	this.wobble += step * wobbleSpeed;
	var wobblePos = Math.sin(this.wobble) * wobbleDist;
	this.pos = this.basePos.plus(new Vector(0,wobblePos));
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
	...
	/// instawin handling
  } else if (type == "instawin") {
    this.actors = this.actors.filter(function(other) {
      return other != actor;
    });
    if (!this.actors.some(function(actor) {
           return actor.type == "instawin";
         })) {
      this.status = "won";
      this.finishDelay = 1;
    }
